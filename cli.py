...
class CLI:
    ...
    def help(self):
        ...
        print("Flags:")
        print(
            "  -d - direct mode - Directly contact database instead of going through REST API"
        )
        ...
        print(
            "    Gets all secrets that the user has access to (requires a private key)"
        )
        print("")
        ...
        print(
            "        optional - if left out, the public key will be written to the console"
        )
        print("")
        ...
        print(
            "        optional - if left out, the encrypted private key will be written to the console"
        )
        print("")
        ...
        print(
            "        optional - if left out, the private key will be written to the console"
        )
        print("")
    ...
    def parse(self):
        ...
        optlist, self.args = getopt.getopt(
            self.args, "du:k:b:s:t:p:j:c", ["force-password-auth"]
        )
    ...
    def initUserConfig(self):
        self.userConfigDir = appdirs.user_data_dir(
    ...
    def getPrivateKey(self):
        if self.privateKey == None:
            if self.privateKeyFile != None and not self.forceGetKey:
                ...
                self.privateKey = self.helper.decryptPrivateKey(
                    self.user, encryptedPrivateKey, self.getPassword()
                )
            else:
                self.privateKey = self.client.getUserPrivateKey(
                    self.user, self.getPassword()
                )
    ...
    def run(self):
        ...
        if command == "list-users":
            ...
            encryptedKey = self.crypto.decode(
                secretEntry["users"][self.user]["encryptedKey"]
            )
            ...
            if self.mode == "direct":
                ...
                if self.privateKeyFile == None:
                    ...
                    print(
                        "export SLSM_AUTH="
                        + base64.b64encode(authToken).decode("utf-8")
                    )
        else:
            self.help()
