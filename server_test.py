...
class TestServer(unittest.TestCase):
    ...
    def testUserCanChangeOwnDisplayName(self):
        ...
        self.assertEqual(
            True,
            srv.updateUser(ctx, "nonadmin", json.dumps({"displayName": "my name"})),
        )
    ...
    def testUserCanChangeOwnPasswordAuth(self):
        ...
        ctx = srv.mockAuthentication("nonadmin")
        self.assertEqual(
            True, srv.updateUser(ctx, "nonadmin", json.dumps({"passwordAuth": "Y"}))
        )
        self.assertEqual("Y", srv.getUser(ctx, "nonadmin")["passwordAuth"])
        self.assertEqual(
            True, srv.updateUser(ctx, "nonadmin", json.dumps({"passwordAuth": "N"}))
        )
    ...
    def testUserCannotEnableSelf(self):
        ...
        self.assertEqual(
            True, srv.updateUser(ctx, "nonadmin", json.dumps({"enabled": "Y"}))
        )
    ...
    def testUserCanDisableSelf(self):
        ...
        adminctx = srv.mockAuthentication("admin")
        self.assertEqual(
            True, srv.updateUser(ctx, "nonadmin", json.dumps({"enabled": "N"}))
        )
        self.assertEqual("N", srv.getUser(adminctx, "nonadmin")["enabled"])
