...
class TestSystem(unittest.TestCase):
    ...
    def testGenerateKeysForUser(self):
        ...
        self.assertEqual(
            True, len(obj.db.getUser("user1")["encryptedPrivateKey"]) > 1024
        )
        ...
        self.assertEqual(
            True,
            not "encryptedPrivateKey" in obj.db.getUser("user1")
            or obj.db.getUser("user1")["encryptedPrivateKey"] == None,
        )
    ...
    def testAddSecretForSingleUser(self):
        ...
        sid = obj.addSecret(
            "user1", "1", obj.crypto.encode(encryptedKey), encryptedSecret, hmac
        )
        ...
        storedEncryptedKey = obj.crypto.decode(
            secretEntry["users"]["user1"]["encryptedKey"]
        )
        ...
        self.assertEqual(
            True,
            obj.crypto.verifyHmac(storedHmacKey, storedEncryptedSecret, storedHmac),
        )
    ...
    def testSharePassword(self):
        ...
        for userdata in [("user1", privKey), ("user2", privKey2)]:
            ...
            storedEncryptedKey = obj.crypto.decode(
                secretEntry["users"][userdata[0]]["encryptedKey"]
            )
